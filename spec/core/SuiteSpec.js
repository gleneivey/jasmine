describe("Suite", function() {
  ...
  it("adds before-all functions in order of needed execution", function() {
    var env = new j$.Env(),
      suite = new j$.Suite({
        env: env,
        description: "I am a suite"
      }),
      outerBefore = jasmine.createSpy('outerBeforeAll'),
      innerBefore = jasmine.createSpy('insideBeforeAll');

    suite.beforeAll(outerBefore);
    suite.beforeAll(innerBefore);

    expect(suite.beforeAllFns).toEqual([innerBefore, outerBefore]);
  });

  it("adds after-all functions in order of needed execution", function() {
    var env = new j$.Env(),
      suite = new j$.Suite({
        env: env,
        description: "I am a suite"
      }),
      outerAfter = jasmine.createSpy('outerAfterAll'),
      innerAfter = jasmine.createSpy('insideAfterAll');

    suite.afterAll(outerAfter);
    suite.afterAll(innerAfter);

    expect(suite.afterAllFns).toEqual([innerAfter, outerAfter]);
  });
